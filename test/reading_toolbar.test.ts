...
test('outline follows current document headings and preserves zoom after a refresh', async () => {
  ...
  try {
    ...
    assert.match(h.toolbar.outline.className, /\bpreview-outline\b/, 'The outline floats over the document');
    (h.toolbar.outline.lastElementChild as HTMLButtonElement).click();
    assert.deepEqual(visited, ['Child']);
    assert.equal(h.toolbar.outline.hidden, false, 'Choosing a heading keeps the floating outline open');
    const escape = () => new h.dom.window.KeyboardEvent('keydown', { key: 'Escape', bubbles: true, cancelable: true });
    const inside = escape();
    (h.toolbar.outline.firstElementChild as HTMLButtonElement).dispatchEvent(inside);
    assert.equal(inside.defaultPrevented, true);
    assert.equal(h.toolbar.outline.hidden, true);
    assert.equal(outline.getAttribute('aria-pressed'), 'false');
    assert.equal(h.dom.window.document.activeElement, outline, 'Escape inside the outline returns to its button');
    outline.click();
    const from_document = escape();
    assert.equal(h.toolbar.keydown(from_document), true, 'Escape from the document closes an open outline');
    assert.equal(from_document.defaultPrevented, true);
    assert.equal(h.toolbar.outline.hidden, true);
    assert.equal(h.toolbar.keydown(escape()), false, 'Escape is left alone once the outline is closed');
    h.control('Zoom in').click();
