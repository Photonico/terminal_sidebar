...
**0.10.0 はプレリリースビルドです。** VS Code **1.106 以降**と、デスクトップまたはリモートの Node.js 拡張機能ホストが必要です。
...
4. **Open Preview…** で `.pdf`、`.md`、`.markdown`、`.tex` を選択します。LaTeX はコンパイル済みの PDF を表示するため、先に普段のツールでビルドしてください。

## エディターと並べて使う

- Vim などで Markdown を保存すると、プレビューが自動更新されます。PDF も再生成後に更新され、閲覧位置が保存されます。
- **Cmd+F / Ctrl+F** で、ターミナルのバッファや PDF・Markdown 全体を検索できます。大文字と小文字の区別、単語単位の検索、正規表現、件数表示、ハイライトに対応しています。PDF の検索にはテキスト層が必要です。
...
- **Change tab marker** で、テーマに連動するタブアイコンを選べます。同じ起動プロファイルのアイコンは別のリポジトリでも引き継がれます。一時的なターミナルとドキュメントのアイコンは、ワークスペースごとに保存されます。
