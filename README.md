...
**0.10.0 is a pre-release build.** Requires VS Code **1.106+** and a desktop or remote Node.js extension host.
...
4. Select **Open Preview…** for `.pdf`, `.md`, `.markdown`, or `.tex`. LaTeX opens an existing compiled PDF; build with your usual tools first.

## Work beside your editor

- Saving Markdown or rebuilding a PDF refreshes its preview, including saves from Vim. Reading positions are remembered.
- **Cmd+F / Ctrl+F** searches terminal buffers and complete PDF or Markdown documents, with case, whole-word, regex, counts, and highlighting. PDF search requires a text layer.
...
- **Change tab marker** selects a themed icon. Startup-profile icons follow the same profile across repositories. Temporary-terminal and document icons stay workspace-local.
