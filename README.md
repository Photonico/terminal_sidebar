...
- Right-click a tab or section heading for **Rename**, **Change tab name color**, **Restart**, **Close**, and **Export**. Terminal content retains its normal text selection and clipboard behaviour.
- **Change tab name color** highlights a name using one of the theme's sixteen terminal ANSI colours. The active tab uses the chosen colour directly; inactive tabs mix 60% chosen colour with 40% of their original text colour, including hover. The picker previews both states; **Reset** restores the default. Choices are saved in the workspace and follow theme changes. Previously saved icon colours become name colours. Status dots and action buttons keep their own colours.
