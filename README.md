...
- **Change tab marker** adds a [Codicon](https://microsoft.github.io/vscode-codicons/dist/codicon.html) before the command-status dot and tab name, with equal spacing. Quick choices are **bookmark**, **tag**, **flag**, **star**, and **ask**; **Others** opens a searchable dropdown of every icon in the bundled catalog, with icon previews. Choose one of the theme's sixteen terminal ANSI colours or the default active/inactive tab foreground colours in the last column: active markers use the full colour, inactive markers blend 60% chosen colour with 40% original foreground. **Apply** saves, **Cancel** discards the draft, and **Remove** clears the marker. Choices are workspace-local and follow theme changes; tab-name text keeps its normal theme colour. Earlier name-colour and special-character marker preferences are discarded.
