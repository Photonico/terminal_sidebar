...
- Right-click a tab or section heading for **Rename**, **Change tab icon**, **Restart**, **Close**, and **Export**. Terminal content retains its normal text selection and clipboard behaviour. Tab icons offer nine geometric shapes in one row: circle, up/right/down/left triangles, diamond, square, right-pointing pentagon and hexagon. All use the current theme's sixteen terminal ANSI colours. The chosen shape and colour token are remembered locally; changing theme updates their appearance. **Remove** clears the icon.
