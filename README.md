...
- Right-click a tab or section heading for **Rename**, **Change tab icon**, **Restart**, **Close**, and **Export**. Terminal content retains its normal text selection and clipboard behaviour. Tab icons offer seven geometric characters in one row: ● ▲ ▶ ▼ ◀ ◆ ■. They follow `terminal.integrated.fontFamily`, falling back to `editor.fontFamily`, with a consistent icon size and the current theme's sixteen terminal ANSI colours. The chosen shape and colour token are remembered locally; changing theme or font updates their appearance. **Remove** clears the icon.
