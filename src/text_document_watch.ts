...
const max_text_document_bytes = 4 * 1024 * 1024;

/** A bounded read also handles a file that grows between stat and read. */
export async function read_text_document(file_path: string, label = 'Document'): Promise<string> {
  const file = await open(file_path, 'r');
  try {
    const stat = await file.stat();
    if (!stat.isFile() || stat.size > max_text_document_bytes) throw new Error(`Choose a ${label} file smaller than 4 MiB.`);
    const bytes = Buffer.alloc(Math.min(max_text_document_bytes + 1, Math.max(stat.size + 1, 8192)));
    const chunks: Buffer[] = [];
    let total = 0;
    for (;;) {
      const remaining = max_text_document_bytes + 1 - total;
      const { bytesRead } = await file.read(bytes, 0, Math.min(bytes.length, remaining), null);
      if (!bytesRead) break;
      total += bytesRead;
      if (total > max_text_document_bytes) throw new Error(`Choose a ${label} file smaller than 4 MiB.`);
    ...
    catch { throw new Error(`${label} preview requires a UTF-8 text file.`); }
  } finally { await file.close(); }
}

/** Watch the directory so Vim's replace-on-save and file recreation both work. */
export class text_document_watch implements vscode.Disposable {
  ...
  constructor(
    ...
    private readonly label = 'Document',
  ) {
  ...
  private async reload(generation: number): Promise<void> {
    try {
      const text = await read_text_document(this.uri.fsPath, this.label);
      if (this.current(generation)) this.changed(text);
    } catch (error) {
      if (!this.current(generation)) return;
      const message = error instanceof Error ? error.message : '';
      this.unavailable(message.startsWith(`Choose a ${this.label}`) || message.startsWith(`${this.label} preview requires`)
        ? message : `${this.label} unavailable. Waiting for the file to be created again. The last preview is kept.`);
    }
  }
