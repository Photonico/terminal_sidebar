...
export function parse_profiles(value: unknown): terminal_profile[] {
  if (!Array.isArray(value) || value.length > 32) throw new Error('Each sidebar accepts an array with at most 32 startup terminals.');
  ...
  return value.map((entry, index) => {
    ...
    if (!is_tab_name(name)) throw new Error(`Sidebar #${index} needs a name of 1–80 characters on one line.`);
...
export function parse_configuration(value: unknown): sidebar_configuration {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('Sidebar configuration must contain left and right startup arrays.');
...
export function read_configuration(value: unknown, legacy_value?: unknown): sidebar_configuration {
  if (value !== undefined) return parse_configuration(value);
  if (legacy_value !== undefined) return { left: [], right: parse_profiles(legacy_value) };
...
export function is_client_message(value: unknown): value is client_message {
  ...
  switch (message.type) {
    case 'ready': case 'settings': case 'trust': case 'select_profile': case 'configure': case 'refresh_shells': case 'add_tab': return true;
    case 'activate': case 'restart': case 'resize': return valid_identifier && valid_size;
    case 'input': return valid_identifier && typeof message.data === 'string' && message.data.length <= 1024 * 1024;
    case 'close_tab': case 'paste': case 'focus': case 'select': return valid_identifier;
    case 'rename_tab': return valid_identifier && is_tab_name(message.name);
    case 'expanded': return valid_identifier && typeof message.expanded === 'boolean';
    case 'export': return valid_identifier && typeof message.text === 'string' && message.text.length <= 1024 * 1024;
    case 'draft_state': return typeof message.configuring === 'boolean' && typeof message.can_undo === 'boolean' && typeof message.can_redo === 'boolean';
    case 'copy': return typeof message.text === 'string' && message.text.length <= 1024 * 1024;
    case 'save':
      try {
